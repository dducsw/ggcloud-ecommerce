...
def display_logistics_perf():
    if data_provider.use_local:
        raw_df = data_provider.query("fact_orders")
        # Local aggregation: filter and calculate averages
        df = raw_df[~raw_df['status'].isin(['Cancelled', 'Processing'])]
        df = df.groupby('status').agg({
            'shipping_duration_days': 'mean',
            'delivery_duration_days': 'mean'
        }).reset_index()
        df.rename(columns={
            'shipping_duration_days': 'avg_shipping_days',
            'delivery_duration_days': 'avg_delivery_days'
        }, inplace=True)
    else:
        query = f"""
        SELECT 
